...
namespace SyndicationServiceLibrary1
{
    ...
    public class Feed1 : IFeed1
    {
        public SyndicationFeedFormatter CreateFeed()
        {
            ...
            Laba4ASPEntities1 wcf = new Laba4ASPEntities1(new Uri("https://localhost:44378/WcfDataService2.svc/"));
            var data = wcf.NOTEs.ToList();
            foreach (NOTE note in data)
            {
                SyndicationItem item = new SyndicationItem("Mark ", Convert.ToString(note.note1), null);
                items.Add(item);
            }
